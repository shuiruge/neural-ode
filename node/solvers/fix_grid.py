...
class FixGridODESolver(ODESolver):
  ...
  def __init__(self, step_fn, dt, dtype=tf.float32, name='fix_grid_solver'):
  ...
  def __call__(self, fn):
    ...
    cast = _cast(self.dtype, name='cast')

    @tf.function
    def forward(t0, t1, x0, name='forward'):
      with tf.name_scope(f'{self.name}_{name}'):
        cast_back = _cast(t0.dtype, name='cast_back')
...
class RK4Solver(FixGridODESolver):
  ...
  def __init__(self, dt, name='rk4_solver', **kwargs):
    super().__init__(rk4_step_fn, dt, name=name, **kwargs)


def rk4_step_fn(f):

  @tf.function
  def step_forward(t, dt, x, name='rk4_step'):
    with tf.name_scope(name):
      k1 = f(t, x)
